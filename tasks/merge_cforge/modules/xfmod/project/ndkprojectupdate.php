<?php
	...
	//include_once(ICMS_ROOT_PATH."/class/nxoopsLDAP.php");
	...
	if (is_object($icmsUser))
	{
		if (!$icmsUser->isAdmin())
		{
			redirect_header(ICMS_URL."/", 1, _NOPERM . "called from ".__FILE__." line ".__LINE__);
			exit();
		}
	}
	else
	...
	if ($op == 'save')
	{
		...
		if (!($group_name == "dirxml" || $group_name == "dirxmllin" || $group_name == "dirxmlsol" || $group_name == "cldap" || $group_name == "cldaplin" || $group_name == "cldapsol" || $group_name == "cldapaix" || $group_name == "cldaphpux" || $group_name == "jldap" || $group_name == "jldapunx" || $group_name == "jvm117b" || $group_name == "jvm13" || $group_name == "jvm141" || $group_name == "jvm142" || $group_name == "odbc" || $group_name == "odbcrw" || $group_name == "php" || $group_name == "php2"))
		{
			...
			// GET GROUP INFO
			///$res = $icmsDB->query("SELECT * "
			//."FROM ".$icmsDB->prefix("xf_groups")." "
			...
			while (list($key, $val) = each($ndk))
			{
				...
				if ($group_name == $val->shortname)
				{
					...
					// GET PACKAGE INFO
					$pres = $icmsDB->query("SELECT * " ."FROM ".$icmsDB->prefix("xf_frs_package")." " ."WHERE group_id='".$group_id."' AND name LIKE '%NDK%'");
					$prows = $icmsDB->getRowsNum($pres);
						...
						while (list($key2, $val2) = each($tempdown))
						{
							...
							)
							{
								if ($val2->getUpdate() == 'Y')
								{
									...
									$rres = $icmsDB->query("SELECT * FROM ".$icmsDB->prefix("xf_frs_release")." " ."WHERE package_id='".$package_id."' ORDER BY 'release_date' DESC");
									 
									$rrows = $icmsDB->getRowsNum($rres);
									//var_dump("<h2>YES</h2>");
									if ($rrows != 0)
									{
										$rel_id = unofficial_getDBResult($rres, 1, 'release_id');
										 
										$fres = $icmsDB->query("SELECT * FROM ".$icmsDB->prefix("xf_frs_file")." " ."WHERE filename LIKE '%.exe' AND release_id='".$rel_id."'");
										 
										$frows = $icmsDB->getRowsNum($fres);
										for($k = 0; $k < $frows; $k++)
										{
								...
								echo "add new file from xml = <strong>".$val2->getModified()." | ".$val2->getName()." | ".$val2->getPath()." | ".$val2->getSize()." | ".time()." | ".$release_id." | ".$package_id."</strong><br/>";
							}
							/*      // DOCUMENTATION
							else if(strpos($val2->getName(),'_doc.') && strpos($package_name,'Documentation')) {
							if ($val2->getUpdate() == 'Y') {
							// if Y, then update the file path of the last file like this to /2004/jun
							$rres = $icmsDB->query("SELECT * FROM ".$icmsDB->prefix("xf_frs_release")." "
							...
							$fres = $icmsDB->query("SELECT * FROM ".$icmsDB->prefix("xf_frs_file")." "
					...
					$query = "DELETE FROM ".$icmsDB->prefix("xf_doc_data")." " ."WHERE title LIKE '%download%'";
					$icmsDB->queryF($query);
					 
					$query = "DELETE FROM ".$icmsDB->prefix("xf_sample_data")." " ."WHERE title LIKE '%download%'";
					$icmsDB->queryF($query);
					 
					// SUBMIT NEWS
					$new_id = forum_create_forum($icmsForge['sysnews'], "Updated ".$val->getName(), 1, 0, '', 0);
					$sql = "INSERT INTO ".$icmsDB->prefix("xf_news_bytes")." " ."(group_id,submitted_by,is_approved,date,forum_id,summary,details) " ."VALUES(" ."'$group_id'," ."'".$icmsUser->getVar("uid")."'," ."'1'," ."'".time()."'," ."'$new_id'," ."'Updated ".$val->getName()."'," ."'".$val->getWhatsNew()."')";
					$icmsDB->queryF($sql);
		...
		else if($group_name == "cldap" || $group_name == "cldaplin" || $group_name == "cldapsol" || $group_name == "cldapaix" || $group_name == "cldaphpux")
		{
			...
			while (list($key, $val) = each($ndk))
			{
				...
				if ($group_name == $val->shortname)
				{
					echo "<h2>".$group_name."</h2>";
					// GET PACKAGE INFO
					$pres = $icmsDB->query("SELECT * " ."FROM ".$icmsDB->prefix("xf_frs_package")." " ."WHERE group_id='".$cldap."' AND name LIKE '%NDK%'");
					$prows = $icmsDB->getRowsNum($pres);
					...
					for($i = 0; $i < $prows; $i++)
					{
						...
						$relres = $icmsDB->query("SELECT * FROM ".$icmsDB->prefix("xf_frs_release")." " ."WHERE name = '".$release_name."' AND package_id='".$package_id."'");
					...
					// SUBMIT NEWS
					$new_id = forum_create_forum($icmsForge['sysnews'], "Updated ".$val->getName(), 1, 0, '', 0);
					$sql = "INSERT INTO ".$icmsDB->prefix("xf_news_bytes")." " ."(group_id,submitted_by,is_approved,date,forum_id,summary,details) " ."VALUES(" ."'".$cldap."'," ."'".$icmsUser->getVar("uid")."'," ."'1'," ."'".time()."'," ."'$new_id'," ."'Updated ".$val->getName()."'," ."'".$val->getWhatsNew()."')";
					$icmsDB->queryF($sql);
					echo "<font color='green'><p/>added news of project update</font><p/>";
				}
			}
		...
		else if($group_name == "jvm117b" || $group_name == "jvm13" || $group_name == "jvm141" || $group_name == "jvm142")
		{
			...
			while (list($key, $val) = each($ndk))
			{
				...
				if ($group_name == $val->shortname)
				{
					echo "<h2>".$group_name."</h2>";
					// GET PACKAGE INFO
					$pres = $icmsDB->query("SELECT * " ."FROM ".$icmsDB->prefix("xf_frs_package")." " ."WHERE group_id='".$jvm."' AND name LIKE '%NDK%'");
					$prows = $icmsDB->getRowsNum($pres);
					...
					// SUBMIT NEWS
					$new_id = forum_create_forum($icmsForge['sysnews'], "Updated ".$val->getName(), 1, 0, '', 0);
					$sql = "INSERT INTO ".$icmsDB->prefix("xf_news_bytes")." " ."(group_id,submitted_by,is_approved,date,forum_id,summary,details) " ."VALUES(" ."'".$jvm."'," ."'".$icmsUser->getVar("uid")."'," ."'1'," ."'".time()."'," ."'$new_id'," ."'Updated ".$val->getName()."'," ."'".$val->getWhatsNew()."')";
					$icmsDB->queryF($sql);
					echo "<font color='green'><p/>added news of project update</font><p/>";
				}
			}
		...
		else if($group_name == "dirxml" || $group_name == "dirxmllin" || $group_name == "dirxmlsol")
		{
			...
			while (list($key, $val) = each($ndk))
			{
				...
				if ($group_name == $val->shortname)
				{
					echo "<h2>".$group_name."</h2>";
					// GET PACKAGE INFO
					$pres = $icmsDB->query("SELECT * " ."FROM ".$icmsDB->prefix("xf_frs_package")." " ."WHERE group_id='".$dirxml."' AND name LIKE '%NDK%'");
					$prows = $icmsDB->getRowsNum($pres);
					...
					for($i = 0; $i < $prows; $i++)
					{
						...
						while (list($key2, $val2) = each($tempdown))
						{
							...
							)
							{
								...
								echo "add new file from xml = <strong>".$val2->getModified()." | ".$file_name." | ".$val2->getPath()." | ".$val2->getSize()." | ".time()." | ".$release_id." | ".$package_id."</strong><br/>";
							}
							/*
							// DOCUMENTATION
							else if(strpos($val2->getName(),'_doc.') && strpos($package_name,'Documentation')) {
					...
					// SUBMIT NEWS
					$new_id = forum_create_forum($icmsForge['sysnews'], "Updated ".$val->getName(), 1, 0, '', 0);
					$sql = "INSERT INTO ".$icmsDB->prefix("xf_news_bytes")." " ."(group_id,submitted_by,is_approved,date,forum_id,summary,details) " ."VALUES(" ."'".$dirxml."'," ."'".$icmsUser->getVar("uid")."'," ."'1'," ."'".time()."'," ."'$new_id'," ."'Updated ".$val->getName()."'," ."'".$val->getWhatsNew()."')";
					$icmsDB->queryF($sql);
					echo "<font color='green'><p/>added news of project update</font><p/>";
				}
			}
		...
		else if($group_name == "jldap" || $group_name == "jldapunx")
		{
			...
			while (list($key, $val) = each($ndk))
			{
				...
				if ($group_name == $val->shortname)
				{
					echo "<h2>".$group_name."</h2>";
					// GET PACKAGE INFO
					$pres = $icmsDB->query("SELECT * " ."FROM ".$icmsDB->prefix("xf_frs_package")." " ."WHERE group_id='".$jldap."' AND name LIKE '%NDK%'");
					$prows = $icmsDB->getRowsNum($pres);
					...
					// SUBMIT NEWS
					$new_id = forum_create_forum($icmsForge['sysnews'], "Updated ".$val->getName(), 1, 0, '', 0);
					$sql = "INSERT INTO ".$icmsDB->prefix("xf_news_bytes")." " ."(group_id,submitted_by,is_approved,date,forum_id,summary,details) " ."VALUES(" ."'".$jldap."'," ."'".$icmsUser->getVar("uid")."'," ."'1'," ."'".time()."'," ."'$new_id'," ."'Updated ".$val->getName()."'," ."'".$val->getWhatsNew()."')";
					$icmsDB->queryF($sql);
					echo "<font color='green'><p/>added news of project update</font><p/>";
				}
			}
		...
		else if($group_name == "odbc" || $group_name == "odbcrw")
		{
			...
			while (list($key, $val) = each($ndk))
			{
				...
				if ($group_name == $val->shortname)
				{
					echo "<h2>".$group_name."</h2>";
					// GET PACKAGE INFO
					$pres = $icmsDB->query("SELECT * " ."FROM ".$icmsDB->prefix("xf_frs_package")." " ."WHERE group_id='".$odbc."' AND name LIKE '%NDK%'");
					$prows = $icmsDB->getRowsNum($pres);
					...
					// SUBMIT NEWS
					$new_id = forum_create_forum($icmsForge['sysnews'], "Updated ".$val->getName(), 1, 0, '', 0);
					$sql = "INSERT INTO ".$icmsDB->prefix("xf_news_bytes")." " ."(group_id,submitted_by,is_approved,date,forum_id,summary,details) " ."VALUES(" ."'".$odbc."'," ."'".$icmsUser->getVar("uid")."'," ."'1'," ."'".time()."'," ."'$new_id'," ."'Updated ".$val->getName()."'," ."'".$val->getWhatsNew()."')";
					$icmsDB->queryF($sql);
					echo "<font color='green'><p/>added news of project update</font><p/>";
				}
			}
		...
		else if($group_name == "php" || $group_name == "php2")
		{
			...
			while (list($key, $val) = each($ndk))
			{
				...
				if ($group_name == $val->shortname)
				{
					echo "<h2>".$group_name."</h2>";
					// GET PACKAGE INFO
					$pres = $icmsDB->query("SELECT * " ."FROM ".$icmsDB->prefix("xf_frs_package")." " ."WHERE group_id='".$php."' AND name LIKE '%NDK%'");
					$prows = $icmsDB->getRowsNum($pres);
					...
					// SUBMIT NEWS
					$new_id = forum_create_forum($icmsForge['sysnews'], "Updated ".$val->getName(), 1, 0, '', 0);
					$sql = "INSERT INTO ".$icmsDB->prefix("xf_news_bytes")." " ."(group_id,submitted_by,is_approved,date,forum_id,summary,details) " ."VALUES(" ."'".$php."'," ."'".$icmsUser->getVar("uid")."'," ."'1'," ."'".time()."'," ."'$new_id'," ."'Updated ".$val->getName()."'," ."'".$val->getWhatsNew()."')";
					$icmsDB->queryF($sql);
					echo "<font color='green'><p/>added news of project update</font><p/>";
				}
			}
	...
	else
	{
		...
		while (list($key, $val) = each($ndk))
		{
			...
			$result = $icmsDB->query("SELECT * FROM ".$icmsDB->prefix("xf_groups")." WHERE unix_group_name='".$val->getShortName()."'");
			...
			if ($rows > 0 && !($val->getShortName() == 'dirxml' || $val->getShortName() == 'cldap' || $val->getShortName() == 'jldap'))
			{
				...
				while (list($key2, $val2) = each($tempdown))
				{
					...
					while (list($key3, $val3) = each($val2))
					{
						if ($key3 != "path")
						{
							echo "<strong>".$key3."</strong>: ".$val3."&nbsp;&nbsp; ";
						}
						else
						{
							echo "</tr><tr><td><strong>".$key3."</strong>: ".$val3;
						}
			...
			{
				if ($val->getShortName() == "dirxml" || $val->getShortName() == "dirxmllin" || $val->getShortName() == "dirxmlsol" || $val->getShortName() == "cldap" || $val->getShortName() == "cldaplin" || $val->getShortName() == "cldapsol" || $val->getShortName() == "cldapaix" || $val->getShortName() == "cldaphpux" || $val->getShortName() == "jldap" || $val->getShortName() == "jldapunx" || $val->getShortName() == "jvm117b" || $val->getShortName() == "jvm13" || $val->getShortName() == "jvm141" || $val->getShortName() == "jvm142" || $val->getShortName() == "odbc" || $val->getShortName() == "odbcrw" || $val->getShortName() == "php" || $val->getShortName() == "php2" )
				{
	...
	{
		global $icmsDB, $ndk, $tempndk, $tempdownload, $val;
		if ($name == "PROJECT")
		{
			if (is_array($attribs))
			{
				while (list($key, $val) = each($attribs))
				{
					if ($key == 'SHORTNAME')
					{
						...
						if ($val == 'ldapcsharp' || $val == 'psqlsdk' || $val == 'apache' || $val == 'mysql' || $val == 'rsync' || $val == 'postgres' || $val == 'cwpdkc' || $val == 'uddiaix' || $val == 'uddisol' || $val == 'uddiwin' || $val == 'uddinw' || $val == 'uddilin' || $val == 'gwsdk' || $val == 'bns-index' || $val == 'dirxml-index' || $val == 'jbrokermq' || $val == 'jbrokertm' || $val == 'jbrokerweb' || $val == 'jbrokerorb' || $val == 'jbrokercon' || $val == 'exworkbench' || $val == 'exappserver' || $val == 'exdirector' || $val == 'excomposer' || $val == 'general' || $val == 'w32sdkc' || $val == 'njsdk' || $val == 'nwsdkc' || $val == 'edirsdk' || $val == 'tomcat' || $val == 'j2eeclientlin' || $val == 'j2eeclientwin' || $val == 'j2eeclientsol' || $val == 'gwsdk' || $val == 'gwover' || $val == 'maccli' || $val == '' || $val == 'bns_cmdover' || $val == 'cwpdk')
						{
						...
						{
							$new = NULL;
							$res = $icmsDB->query("SELECT group_id " ."FROM ".$icmsDB->prefix("xf_groups")." " ."WHERE unix_group_name='".$val."'");
	...
	function tag_contents($parser, $data)
	{
		global $tempndk;
		// echo $data;
		if ($tempndk->attribute == "Name")
		{
			$tempndk->setName($data);
		}
		elseif($tempndk->attribute == "Description")
		{
			$tempndk->setDescription($data);
		}
		elseif($tempndk->attribute == "Dependencies")
		{
			$tempndk->setDependencies($data);
		}
		elseif($tempndk->attribute == "Whats New")
		{
			$tempndk->setWhatsNew($data);
		}
		elseif($tempndk->attribute == "Whats New Archive")
		{
			$tempndk->setWhatsNewArchive($data);
		}
		elseif($tempndk->attribute == "Support Status")
		{
			$tempndk->setSupportStatus($data);
		}
	}
