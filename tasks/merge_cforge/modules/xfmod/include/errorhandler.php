...
class IcmsForgeErrorHandler {
	var $messages;
	var $errors;
	var $system_error;
	 
	function IcmsForgeErrorHandler()
	{
		$messages = array();
		$errors = array();
		$system_error = "";
	...
	function getDisplayFeedback()
	{
		...
		{
			$content .= "<div style='font-weight:bold;color:#0000DD'>" .implode("<br>", $this->messages)
			."</div>";
			$this->messages = array();
		}
		if (count($this->errors) > 0)
		{
			$content .= "<div style='font-weight:bold;color:#FF0000'>" .implode("<br>", $this->errors)
			."</div>";
			$this->errors = array();
		}
		return $content;
...
$icmsForgeErrorHandler = new IcmsForgeErrorHandler();
?>