...
$sql = 'SELECT p.name as package_name,r.name as release_name,f.file_id,f.filename,t.target FROM '
	...
	.$xoopsDB->prefix('xf_frs_target').' as t LEFT JOIN '
	.$xoopsDB->prefix('xf_webservice_publish').' as w ON (f.file_id = w.file_id)'
	." WHERE g.unix_group_name='$name'"
	.' AND g.group_id=p.group_id AND p.package_id=r.package_id AND r.release_id=f.release_id AND f.file_id=t.file_id'
	." AND (w.file_id IS NULL OR w.status = 'failed')"
	.' ORDER BY p.package_name,r.release_date DESC,r.release_name,f.filename,t.target';
...
if (!$result)
{
	...
	$tpl = TEMPLATE_CONTEXT.'error.tpl';
	$args['id'] = 300;
	$args['contents'] = _XFWEBSERVICE_ERROR_300.': '.$xoopsDB->error();
	echo fill_template($tpl, $args);
	return;
}
...
while (list($pName, $rName, $fID, $fName, $target) = $xoopsDB->fetchRow($result))
{
