...
function get_priority_colors_key() {
	...
	$content = "<P><B>"._XF_PRIORITYCOLORS.":</B><BR>"
	    ."<TABLE BORDER='0'><TR>";
...
function validate_email ($address) {
  ...
  $return=true;
  if (($xoopsForge['validate_email'] >= 1) && ($return == true))
    $return = (ereg('^[-!#$%&\'*+\\./0-9=?A-Z^_A-z{|}~]+'.'@'.
               '[-!#$%&\'*+\\/0-9=?A-Z^_A-z{|}~]+\.'.
               '[-!#$%&\'*+\\./0-9=?A-Z^_A-z{|}~]+$',$address));
  if (($xoopsForge['validate_email'] >= 2) && ($return == true)) {
...
function checkXFAdminPermissions ($error_on_failure='Access Denied')
{
  ...
  include_once($xoopsConfig['root_path']."class/xoopsmodule.php");
	include_once($xoopsConfig['root_path']."kernel/group.php");
...
function xoopsForgeMail ($from, $fromname, $subject, $body, $to_arr, $bcc_arr=false, $cc_arr=false){
	...
	if($bcc_arr) $xoopsMailer->setToEmails($bcc_arr);
	if($cc_arr) $xoopsMailer->setToEmails($cc_arr);
...
function  ShowResultSet($result,$title="Untitled",$linkify=false)  {
    ...
    $content = "<table border='0' width='100%'>"
		    ."<tr class='bg2'><td colspan='".$cols."'><B style='font-size:14px;text-align:left;'>".$title."</B></td></tr>";
