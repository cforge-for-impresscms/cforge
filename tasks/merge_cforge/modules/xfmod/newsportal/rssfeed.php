<?php
	...
	for($i = 0; $i < 10; $i++)
	{
		...
		else
		{
			...
			$feeds[$i]['link'] = ICMS_URL."/modules/xfmod/newsportal/article.php?group_id=$group_id&msg_id=".($msg_id-$i)."&group=$group";
			$feeds[$i]['pubDate'] = date("r", $message->header->date);
			$feeds[$i]['comments'] = $feeds[$i]['link'];
		}
		 
	}
	$sql = "SELECT group_name, short_description FROM ".$icmsDB->prefix("xf_groups")." WHERE group_id=".$group_id;
	$result = $icmsDB->query($sql);
	list($project_name, $project_desc) = $icmsDB->fetchRow($result);
	 
	$sql = "SELECT forum_desc_name FROM ".$icmsDB->prefix("xf_forum_nntp_list")." WHERE forum_name='".$group."'";
	$result = $icmsDB->query($sql);
	list($forum_name) = $icmsDB->fetchRow($result);
	header("Content-Type: text/xml");
	echo '<?xml version="1.0" ?>';
	echo '<rss version="2.0">';
	echo '<channel>';
	echo '<title>Novell Forge : Project - '.$project_name.' : Forum - '.$forum_name.'</title>';
	echo '<link>'.ICMS_URL.'/modules/xfmod/newsportal/?group_id='.$group_id.'</link>';
