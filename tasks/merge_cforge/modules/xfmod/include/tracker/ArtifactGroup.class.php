	...
	class ArtifactGroup extends Error {
		...
		*/
		function create($name)
		{
			...
			$sql = "INSERT INTO ".$this->db->prefix("xf_artifact_group")."(group_artifact_id,group_name) " ."VALUES('".$this->ArtifactType->getID()."','".$ts->makeTboxData4Save($name)."')";
		...
		*/
		function update($name)
		{
			...
			$sql = "UPDATE ".$this->db->prefix("xf_artifact_group")." SET group_name='".$ts->makeTboxData4Save($name)."' " ."WHERE id='". $this->getID() ."' " ."AND group_artifact_id='". $this->ArtifactType->getID() ."'";
