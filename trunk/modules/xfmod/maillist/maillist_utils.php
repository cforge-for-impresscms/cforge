...
function maillist_add(&$project,$listname,$listdesc){
	...
	if((maillist_count($project->getID()) >= $xoopsForge['max_maillists']) && (!$xoopsUser->isAdmin())){
		return "You may only create ".$xoopsForge['max_maillists']." mailing lists for your project."  
						."  If you would like to create more please contact the site administrator";
	}
	$maillist_query_result = $xoopsDB->query("SELECT u.uname,u.email "
						  ."FROM ".$xoopsDB->prefix("users")." u,".$xoopsDB->prefix("xf_user_group")." ug "
						  ."WHERE u.uid=ug.user_id "
						  ."AND ug.group_id=".$project->getID()." "
...
function maillist_delete(&$project,$id){
	...
	list($suffix) = $xoopsDB->fetchRow($result);
...
function maillist_send_confirmation( $listname, $listdesc, $unixname, $owner, $owner_email, $is_project=true )
{
	...
	$msg_body = $owner . ":\n\nThank you for registering your new $prjtype, $unixname"
    	. ", with $site.\n\n"
	...
	$msg_body .= "\nThe password for your mailing list is the name of the list followed by \"-passwd\", i.e. "
    	. "for the " . $unixname . "-" . $listname . " mailing list, the password is " . $unixname . "-" . $listname . "-passwd.  "
