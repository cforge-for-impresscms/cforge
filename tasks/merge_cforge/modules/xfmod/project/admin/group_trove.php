<?php
	...
	*/
	...
	$icmsOption['template_main'] = 'project/admin/xfmod_group_trove.html';
	...
	if ($submit && $root1)
	{
		...
		while (list($rootnode, $value) = each($root1))
		{
			...
			$icmsDB->queryF("DELETE FROM ".$icmsDB->prefix("xf_trove_group_link")." " ."WHERE group_id='$group_id' " ."AND trove_cat_root='$rootnode'");
			...
			for($i = 1; $i <= $TROVE_MAXPERROOT; $i++)
			{
				...
				[$rootnode])
				{
					trove_setnode($group_id, $ {
						$varname }
					[$rootnode], $rootnode);
				}
			}
		}
		redirect_header(ICMS_URL."/modules/xfmod/project/admin/?group_id=".$group_id, 0, _XF_PRJ_GROUPTROVEALTERED);
		exit;
	}
	...
	if ($group->isFoundry())
	{
		...
		$content .= "<p>"._XF_COMM_EDITTROVEEXPLAIN1."</p>" ."<p>"._XF_COMM_EDITTROVEEXPLAIN2."</p>" ."<form action='".$_SERVER['PHP_SELF']."' method='POST'>";
	}
	else
	{
		$content = "<H4>"._XF_PRJ_EDITTROVECATEGORIZATION."</H4>";
		$content .= "<p>"._XF_PRJ_EDITTROVEEXPLAIN1."</p>" ."<p>"._XF_PRJ_EDITTROVEEXPLAIN2."</p>" ."<form action='".$_SERVER['PHP_SELF']."' method='POST'>";
	}
	 
	$CATROOTS = trove_getallroots($group->isFoundry());
	while (list($catroot, $fullname) = each($CATROOTS))
	{
		$content .= "<HR><p><strong>".$fullname."</strong>";
		 
		$res_grpcat = $icmsDB->queryF("SELECT trove_cat_id " ."FROM ".$icmsDB->prefix("xf_trove_group_link")." " ."WHERE group_id='$group_id' " ."AND trove_cat_root='$catroot'");
		...
		for($i = 1; $i <= $GLOBALS['TROVE_MAXPERROOT']; $i++)
		{
			...
			if ($row_grpcat = $icmsDB->fetchArray($res_grpcat))
			{
	...
	echo "<br><hr><br><strong>Community</strong><br>";
	 
	$res_grpcat = $icmsDB->queryF("SELECT trove_cat_id "
	."FROM ".$icmsDB->prefix("xf_trove_group_link")." "
