...
<?php
	...
	db_query("INSERT INTO ".$dbprefix."_xf_user_stats SELECT u.uid" .", COALESCE(p.count,0) as projects" .", COALESCE(c.count,0) as cvs" .", COALESCE(t.count,0) as trackers" .", COALESCE(n.count,0) as news" .", COALESCE(s.count,0) as samples" .", COALESCE(d.count,0) as documents" ." FROM ".$dbprefix."_users AS u" ." LEFT JOIN projects AS p USING(uid)" ." LEFT JOIN cvs AS c USING(uid)" ." LEFT JOIN trackers AS t USING(uid)" ." LEFT JOIN news AS n USING(uid)" ." LEFT JOIN samples AS s USING(uid)" ." LEFT JOIN documents AS d USING(uid)");
