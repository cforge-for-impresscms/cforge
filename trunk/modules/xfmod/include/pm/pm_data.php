...
function pm_data_get_tasks ($group_project_id) {
  ...
  if (!$PM_DATA_TASKS["$group_project_id"]) {
    $sql = "SELECT project_task_id,summary "
          ."FROM ".$xoopsDB->prefix("xf_project_task")." "
          ."WHERE group_project_id='$group_project_id' "
          ."AND status_id <> '3' ORDER BY project_task_id DESC";

    $PM_DATA_TASKS["$group_project_id"] = $xoopsDB->query($sql,100);
  }
...
function pm_data_get_subprojects ($group_id) {
  ...
  if (!$PM_DATA_SUBPROJECTS["$group_id"]) {
    $sql = "SELECT group_project_id,project_name "
          ."FROM ".$xoopsDB->prefix("xf_project_group_list")." WHERE group_id='$group_id'";

    $PM_DATA_SUBPROJECTS["$group_id"] = $xoopsDB->query($sql);
  }
...
function pm_data_get_technicians ($group_id) {
  ...
  if (!$PM_DATA_TECHNICIANS["$group_id"]) {
    $sql = "SELECT u.uid,u.uname "
          ...
          ."AND ug.project_flags IN (1,2) "
          ."ORDER BY u.uname";
    $PM_DATA_TECHNICIANS["$group_id"] = $xoopsDB->query($sql);
  }
...
function pm_data_get_dependent_tasks ($project_task_id) {
  ...
  $sql = "SELECT is_dependent_on_task_id "
        ."FROM ".$xoopsDB->prefix("xf_project_dependencies")." "
...
function pm_data_get_assigned_to ($project_task_id) {
  ...
  $sql = "SELECT assigned_to_id "
        ."FROM ".$xoopsDB->prefix("xf_project_assigned_to")." "
        ."WHERE project_task_id='$project_task_id'";
...
function pm_data_create_history ($field_name,$old_value,$project_task_id) {
  ...
  $sql = "INSERT INTO ".$xoopsDB->prefix("xf_project_history")." (project_task_id,field_name,old_value,mod_by,date) "
        ."VALUES ('$project_task_id','$field_name','$old_value','".$xoopsUser->getVar("uid")."','".time()."')";
...
function pm_data_insert_assigned_to($array,$project_task_id) {
  ...
  if ($user_count < 1) {
    ...
    $sql = "INSERT INTO ".$xoopsDB->prefix("xf_project_assigned_to")." (project_task_id,assigned_to_id) "
...
function pm_data_insert_dependent_tasks($array,$project_task_id) {
  ...
  if ($depend_count < 1) {
    ...
    $sql = "INSERT INTO ".$xoopsDB->prefix("xf_project_dependencies")." (project_task_id,is_dependent_on_task_id) "
          ."VALUES ('$project_task_id','100')";
...
function pm_data_create_task ($group_project_id,$start_month,$start_day,$start_year,$end_month,$end_day,
  ...
  $sql = "INSERT INTO ".$xoopsDB->prefix("xf_project_task")." "
        ."(group_project_id,summary,details,percent_complete,priority,hours,start_date,end_date,created_by,status_id) VALUES ("
        ."'$group_project_id',"
...
function pm_data_update_task ($group_project_id,$project_task_id,$start_month,$start_day,$start_year,
  ...
  */
  $sql = "UPDATE ".$xoopsDB->prefix("xf_project_task")." SET "
        ."status_id='$status_id',"
        ."priority='$priority',"
...
function mail_followup($project_task_id,$group_project_id,$more_addresses=false,$new_task=0) {
	...
	$sql = "SELECT pt.*, pgl.*, g.group_name,g.new_task_address,g.unix_group_name, ".
	       "g.send_all_tasks,ps.status_name,u.email, ".
				 ...
				 "FROM ".$xoopsDB->prefix("xf_project_task")." pt,".$xoopsDB->prefix("xf_project_group_list")." pgl,".$xoopsDB->prefix("xf_project_status")." ps,".$xoopsDB->prefix("users")." u,".$xoopsDB->prefix("xf_groups")." g ".
				 "WHERE project_task_id='$project_task_id' ".
	...
	if ($result && $xoopsDB->getRowsNum($result) > 0) {
		...
		$sql = "SELECT u.email AS Email,u.uname ".
		       "FROM ".$xoopsDB->prefix("users")." u,".$xoopsDB->prefix("xf_project_assigned_to")." pat ".
					 "WHERE pat.project_task_id='$project_task_id' ".
					 "AND u.level <> 0 ".
					 "AND u.uid=pat.assigned_to_id";
		...
		*/
		...
		$body .= "\n\n-------------------------------------------------------".
			       "\n"._XF_PM_FORMOREINFO.
			       "\n\n".XOOPS_URL."/modules/xfmod/pm/task.php?func=detailtask&project_task_id=".
						 unofficial_getDBResult($result,0,'project_task_id')."&group_id=".
						 unofficial_getDBResult($result,0,'group_id')."&group_project_id=".unofficial_getDBResult($result,0,'group_project_id');

		$subject = "[ ".unofficial_getDBResult($result,0,'unix_group_name')."-"._XF_PM_TASK." #".unofficial_getDBResult($result,0,'project_task_id').'] '.
			$ts->makeTareaData4Edit(unofficial_getDBResult($result,0,'summary'));
