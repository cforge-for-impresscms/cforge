<?php
	...
	if (!$file_id)
	{
		redirect_header(ICMS_URL."/", 4, "Invalid File");
		exit;
	}
	if (isset($_POST['submit']) && isset($_POST['targets']))
	{
		...
		{
			$sql = "INSERT INTO ".$icmsDB->prefix('xf_frs_target')."(file_id, target) VALUES($file_id, '$target')";
			$icmsDB->queryF($sql);
		}
		...
		$sql = "SELECT target FROM ".$icmsDB->prefix('xf_frs_target')." WHERE file_id=$file_id";
		$result = $icmsDB->query($sql);
		while (list($target) = $icmsDB->fetchRow($result))
		{
			$file_targets[] = $target;
		}
	?>
