...
if (!$icmsUser)
{
	redirect_header($_SERVER["HTTP_REFERER"], 4, _NOPERM . "called from ".__FILE__." line ".__LINE__);
	exit;
}
 
if (!$survey_id || !$group_id)
{
	redirect_header($_SERVER["HTTP_REFERER"], 4, "Error<br />For some reason, the Group ID or Survey ID did not make it to this page");
	exit;
}
...
$result = $icmsDB->queryF("DELETE FROM ".$icmsDB->prefix("xf_survey_responses")." " ."WHERE survey_id='" . addslashes($survey_id) . "' " ."AND group_id='" . addslashes($group_id) . "' " ."AND user_id='".$icmsUser->getVar("uid")."'");
...
for($i = 0; $i < $count; $i++)
{
	...
	*/
	...
	$sql = "INSERT INTO ".$icmsDB->prefix("xf_survey_responses")."(user_id,group_id,survey_id,question_id,response,date) " ."VALUES('".$icmsUser->getVar("uid")."','" . addslashes($group_id) . "','" . addslashes($survey_id) . "','" . addslashes($quest_array[$i]) . "','". addslashes($$val) . "','$now')";
