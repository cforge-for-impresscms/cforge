...
<?php
	...
	******************************************************************************/
	...
	function deactivate_project($group_id, $timeframe)
	{
		...
		$sql = "UPDATE ".$dbprefix."_xf_groups" . " SET status='N', is_public='0'" . " WHERE group_id='".$group_id."'";
		if ($debug)
		{
		...
		$sql = "SELECT group_name, unix_group_name" . " FROM ".$dbprefix."_xf_groups" . " WHERE group_id='".$group_id."'";
		$result = db_query($sql);
		$row = db_fetch_array($result);
		$group_name = $row['group_name'];
		$unix_name = $row['unix_group_name'];
		$sql = "SELECT email" . " FROM ".$dbprefix."_users" . " WHERE level='5'";
		$result = db_query($sql);
		$row = db_fetch_array($result);
		$admin_email = $row['email'];
		 
		$emailmsg = "The purpose of this e-mail is to notify you" . " that your project, ".$group_name."," . " has been changed to an inactive status." . "  You are receiving this e-mail because your" . " are shown to be an administrator of this project.<br>" . "This has been done simply because this project" . " does not appear to have had any activity within" . " the past ".$timeframe." months.  The criteria by" . " which this decision was made include:<ul>" . " <li>An activity rating of 0</li>" . " <li>No project activity recorded in project history" . " in the past ".$timeframe." months</li>" . " <li>No downloads of any project files in the" . " past ".$timeframe." months</li></ul>" . "You can easily reactivate your project by" . " returning to your project page" . " and clicking on the \"Reactivate This Project\"" . " button.  The project will then become active again.<br>" . "You may e-mail the site administrator at " . "<a href=\"mailto:".$admin_email."\">" . $admin_email . "</a> if you have any questions.";
		 
		$sql2 = "SELECT u.email, u.name" . " FROM ".$dbprefix."_users u, ".$dbprefix."_xf_user_group ug" . " WHERE ug.group_id='".$group_id."' AND u.uid=ug.user_id";
	...
	$sql = "SELECT pwm.group_id FROM ".$dbprefix."_xf_project_weekly_metric pwm, ".$dbprefix."_xf_groups g" ." WHERE pwm.percentile <= '".$min_activity_percentile."'" ." AND g.group_id=pwm.group_id" ." AND g.status!='N'" ." AND g.is_public='1'";
	if ($debug)
	{
		echo "Asking database:  $sql?\n";
	}
	...
	while ($row = db_fetch_array($result1))
	{
		...
		if (0 == db_numrows($result2))
		{
			...
			{
				$real_first_month = 12 -($now['tm_mon'] - $activity_months);
