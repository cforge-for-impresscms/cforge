...
$xoopsOption['template_main'] = 'project/admin/xfmod_userpermedit.html';
		...
		if ($add_all) {
		  $sql = "SELECT agl.group_artifact_id "
			      ."FROM ".$xoopsDB->prefix("xf_artifact_group_list")." agl "
						."LEFT JOIN ".$xoopsDB->prefix("xf_artifact_perm")." ap ON agl.group_artifact_id=ap.group_artifact_id "
						."AND ap.user_id='".$u->getVar("uid")."' "
...
$content .= "
	...
	if ( $group->isProject() ) {
	    ...
	    //
	    $res = $xoopsDB->query("SELECT agl.group_artifact_id,agl.name,agl.description,agl.group_id,ap.user_id,ap.perm_level "
				   ."FROM ".$xoopsDB->prefix("xf_artifact_perm")." ap, ".$xoopsDB->prefix("xf_artifact_group_list")." agl "
	    ...
	    for ($i=0; $i<$rows; $i++) {
	      $content .= "<INPUT TYPE='HIDDEN' NAME='updateperms[".$i."][0]' VALUE='".unofficial_getDBResult($res,$i,'group_artifact_id')."'>"
		."<TR class='".($i%2>0?"bg1":"bg3")."'>"
		."<TD>".unofficial_getDBResult($res,$i,'name')." <a href=\"javascript:openHelpWindow('"._XF_PRJ_TRACKER_HELP."')\"><img src='".XOOPS_URL."/modules/xfmod/images/help.gif' width='14' height='14' alt='Help'></a></TD>"
		."<TD><FONT size='-1'><SELECT name='updateperms[".$i."][1]'>"
		."<OPTION value='0'".(unofficial_getDBResult($res,$i,'perm_level')==0?" selected":"").">-"
		."<OPTION value='1'".(unofficial_getDBResult($res,$i,'perm_level')==1?" selected":"").">"._XF_PRJ_TECHNICIAN
		."<OPTION value='2'".(unofficial_getDBResult($res,$i,'perm_level')==2?" selected":"").">"._XF_PRJ_ADMINTECH
		."<OPTION value='3'".(unofficial_getDBResult($res,$i,'perm_level')==3?" selected":"").">"._XF_PRJ_ADMINONLY
		."</SELECT></FONT> <INPUT TYPE='CHECKBOX' NAME='deletefrom[]' VALUE='".unofficial_getDBResult($res,$i,'group_artifact_id')."'> "._XF_G_REMOVE."</TD>"
		."</TR>";
	    }
	}
	...
	if ( $group->isProject() ) {
		...
		$content .=  "<P>";
		$content .= "<INPUT type='submit' name='submit' value='"._XF_PRJ_ADDTOTRACKER."'>&nbsp;"
				."<INPUT type='checkbox' name='add_all' value='1'> "._XF_PRJ_ADDTOALL."</FORM>"
				."</CENTER>";
	}
}
