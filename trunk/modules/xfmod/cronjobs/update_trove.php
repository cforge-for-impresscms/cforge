...
$sql = "INSERT INTO ".$dbprefix."_xf_trove_agg (trove_cat_id,group_id,group_name,unix_group_name,status,register_time,short_description,percentile,ranking) "
      ."SELECT tgl.trove_cat_id,g.group_id,g.group_name,g.unix_group_name,g.status,g.register_time,"
			."g.short_description "
			.",pwm.percentile,pwm.ranking "
			."FROM ".$dbprefix."_xf_groups g "
			."LEFT JOIN ".$dbprefix."_xf_project_weekly_metric pwm USING (group_id)"
			.",".$dbprefix."_xf_trove_group_link tgl "
			."WHERE tgl.group_id=g.group_id "
			."AND (g.is_public=1) "
			."AND (g.type=1) "
			."AND (g.status='A') "
			."ORDER BY trove_cat_id ASC"
			.",ranking ASC ";
db_query($sql);
echo db_error();
...
$sql = "SELECT tc.trove_cat_id,tc.parent,count(g.group_id) AS count "
      ."FROM ".$dbprefix."_xf_trove_cat tc "
      ."LEFT JOIN ".$dbprefix."_xf_trove_group_link tgl ON tc.trove_cat_id=tgl.trove_cat_id "
      ."LEFT JOIN ".$dbprefix."_xf_groups g ON g.group_id=tgl.group_id "
      ."WHERE (g.status='A' OR g.status IS NULL) "
      ."AND ( g.type='1' OR g.status IS NULL) "
      ."AND ( g.is_public='1' OR g.is_public IS NULL) "
      ."GROUP BY tc.trove_cat_id,tc.parent";
$res=db_query($sql);
$rows = db_numrows($res);
