...
$xoopsOption['template_main'] = 'project/xfmod_index.html';
...
if ($project->isInactive() && $activate!='y' && $perm->isSuperUser())
{
	$inactiveInfo = "\n<center>\n\t<p><b>"._XF_PRJ_THISPRJISINACTIVE."</b></p>\n"
...
$sql = "SELECT percentile"
	." FROM ".$xoopsDB->prefix("xf_project_weekly_metric")
	." WHERE group_id=$group_id";
$rs_activity = $xoopsDB->query($sql);
list($activity) = $xoopsDB->fetchArray($rs_activity);
if (!$activity)
{
	$activity = 0;
}
...
$sql = "SELECT SUM(count) as total"
	." FROM ".$xoopsDB->prefix("xf_cvs_commit_tracker")
	." WHERE unix_group_name='$unixname'";
$rs_cvs = $xoopsDB->query($sql);
list($cvscommits) = $xoopsDB->fetchRow($rs_cvs);
if (!$cvscommits)
{
	$cvscommits = 0;
}
...
$sql = "SELECT DISTINCT name"
	...
	.", ".$xoopsDB->prefix("xf_people_job_category")." pjc"
	." WHERE pj.category_id=pjc.category_id"
	." AND pj.status_id=1"
...
if ($rs_jobs)
{
	...
	if ($num > 0)
	{
		...
		if ($num == 1)
		{
			list($jobName) = $xoopsDB->fetchRow($rs_jobs);
...
$sql = "SELECT p.package_id,p.name AS package_name,r.name AS release_name,r.release_id AS release_id,r.release_date AS release_date"
	." FROM ".$xoopsDB->prefix("xf_frs_package")." p"
	.", ".$xoopsDB->prefix("xf_frs_release")." r"
...
else
{
	...
	$lastID = false;
	while ($row = $xoopsDB->fetchArray($rs_frs))
	{
		...
		if ($lastID != $row['package_id'])
		{
			...
			$content .= "<tr class='bg3' align='center'>\n\t<td align='left'><b>"
				.$ts->makeTboxData4Show($row['package_name'])
				."</b></td>\n\t<td>"
				."<a style='text-decoration:underline' href='/modules/xfmod/project/showfiles.php?group_id=$group_id&release_id=".$row['release_id']."#selected'>"
				.$ts->makeTboxData4Show($row['release_name'])
				."</a></td>\n\t<td>"
				.$rel_date['month']." ".$rel_date['mday'].", ".$rel_date['year']
				."</td>\n\t<td>"
				."<a href='/modules/xfmod/project/shownotes.php?group_id=$group_id&release_id=".$row['release_id']
				."'><img src='/modules/xfmod/images/ic/manual16c.png' width='15' height='15' alt='"._XF_PRJ_RELEASENOTES
				."'></a> / <a href='/modules/xfmod/project/filemodule_monitor.php?filemodule_id=".$row['package_id']
				."'><img src='/modules/xfmod/images/ic/mail16d.png' width='15' height='15' alt='"._XF_PRJ_MONITORTHISPACKAGE
				."'></a></td>\n</tr>\n";
		}
	}
}
...
if ($project->usesNews())
{
	$sql = "SELECT u.uname, u.name, nb.forum_id, nb.summary, nb.date, nb.details"
		...
		.", ".$xoopsDB->prefix("xf_news_bytes")." nb"
	...
	{
		while ($row = $xoopsDB->fetchArray($rs_news))
		{
...
if ($project->usesDocman())
{
	$sql = "SELECT u.uid, u.uname, u.name, d.docid, d.title, d.description, d.updatedate"
		." FROM  ".$xoopsDB->prefix("xf_doc_data")." d"
		.", ".$xoopsDB->prefix("xf_doc_groups")." g"
	...
	{
		while ($row = $xoopsDB->fetchArray($rs_docs))
		{
			// Get the user's name and date.
			$name = (empty($row['name']) ? $row['uname'] : $row['name']);
...
// Member Information
$sql = "SELECT u.uid, u.uname, u.name"
	." FROM ".$xoopsDB->prefix("users")." u"
...
<tr>
	<td colspan='2'><p>";
while ($row = $xoopsDB->fetchArray($rs_admins))
{
	$name = (empty($row['name']) ? $row['uname'] : $row['name']);
...
{
	$sql = "SELECT forum_name, forum_desc_name"
	...
	{
		while ($row = $xoopsDB->fetchArray($rs_forums))
		{
...
if ($project->usesMail())
{
	$sql = "SELECT name, description"
		." FROM ".$xoopsDB->prefix("xf_maillists")
		." WHERE group_id=$group_id"
		." ORDER BY name ASC";
	...
	{
		while ($row = $xoopsDB->fetchArray($rs_lists))
		{
	...
	<tr>
		...
		<td align='right'>[&nbsp;<a style='text-decoration:underline' href='/modules/xfmod/maillist/archbrowse.php/$unixname-$suffix/?id=$group_id&prjname=$unixname&mlname=$suffix'>"._XF_PRJ_VIEW_ARCHIVE."</a>&nbsp;/&nbsp;<a style='text-decoration:underline' href='/modules/xfmod/maillist/subscribe.php?group_id=$group_id&list=".urlencode("$unixname-$suffix")."'>"._XF_PRJ_SUBSCRIBE."</a>&nbsp;]</td>
	</tr>
...
$sql = "SELECT g.unix_group_name AS community_unixname, g.group_name AS community_name, g.short_description"
	." FROM ".$xoopsDB->prefix("xf_groups")." g"
	.", ".$xoopsDB->prefix("xf_trove_group_link")." t"
...
{
	while ($row = $xoopsDB->fetchArray($rs_communities))
	{
