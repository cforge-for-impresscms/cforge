<?php
	...
	*
	* @version   $Id: ArtifactCategory.class,v 1.1.1.1 2003/08/01 19:13:48 devsupaul Exp $
	*
	*/
	...
	class ArtifactCategory extends Error {
		...
		*/
		function create($name, $auto_assign_to)
		{
			...
			$sql = "INSERT INTO ".$this->db->prefix("xf_artifact_category")."(group_artifact_id,category_name,auto_assign_to) " ."VALUES('".$this->ArtifactType->getID()."','".$ts->makeTboxData4Save($name)."','$auto_assign_to')";
		...
		*/
		function update($name, $assign_to)
		{
			...
			$sql = "UPDATE ".$this->db->prefix("xf_artifact_category")." " ."SET category_name='".$ts->makeTboxData4Save($name)."',auto_assign_to='$assign_to' " ."WHERE id='". $this->getID() ."' " ."AND group_artifact_id='".$this->ArtifactType->getID()."'";
