...
$xoopsOption['template_main'] = 'xfaccount_index.html';
if (!$xoopsUser)
{
...
$myprojects_sql = "SELECT g.group_name,g.group_id,g.unix_group_name,g.status,g.type,ug.admin_flags,g.is_public "." FROM ".$xoopsDB -> prefix("xf_groups")." g,".$xoopsDB -> prefix("xf_user_group")." ug "." WHERE g.group_id=ug.group_id "." AND ug.user_id='".$xoopsUser -> getVar("uid")."' "." AND g.status='A'"." ORDER BY g.group_name";
...
$sql = "SELECT g.group_name,agl.name,agl.group_id,a.group_artifact_id,a.assigned_to,a.summary,a.artifact_id,a.priority "."FROM ".$xoopsDB -> prefix("xf_artifact")." a, ".$xoopsDB -> prefix("xf_groups")." g, ".$xoopsDB -> prefix("xf_artifact_group_list")." agl "."WHERE a.group_artifact_id=agl.group_artifact_id "."AND agl.group_id=g.group_id "."AND g.status = 'A' "."AND a.assigned_to='".$xoopsUser -> getVar("uid")."' "."AND a.status_id='1' "."ORDER BY agl.group_id,a.group_artifact_id,a.assigned_to,a.status_id";
...
else
{
	...
	for ($i = 0; $i < $rows; $i ++)
	{
		...
		{
			$content.= "<tr><td colspan=2><b><a href='../xfmod/tracker/?group_id=".unofficial_getDBResult($result, $i, 'group_id')."&atid=".unofficial_getDBResult($result, $i, 'group_artifact_id')."'>".$ts -> makeTboxData4Show(unofficial_getDBResult($result, $i, 'group_name'))." - ".$ts -> makeTboxData4Show(unofficial_getDBResult($result, $i, 'name'))."</a></td></tr>";
		}
...
$sql = "SELECT g.group_name,agl.name,agl.group_id,a.group_artifact_id,a.assigned_to,a.summary,a.artifact_id,a.priority "."FROM ".$xoopsDB -> prefix("xf_artifact")." a, ".$xoopsDB -> prefix("xf_groups")." g, ".$xoopsDB -> prefix("xf_artifact_group_list")." agl "."WHERE a.group_artifact_id=agl.group_artifact_id "."AND agl.group_id=g.group_id "."AND g.status = 'A' "."AND a.submitted_by='".$xoopsUser -> getVar("uid")."' "."AND a.status_id='1' "."ORDER BY agl.group_id,a.group_artifact_id,a.submitted_by,a.status_id";
...
$sql = "SELECT g.group_name,g.unix_group_name,g.group_id,p.name,f.filemodule_id "."FROM ".$xoopsDB -> prefix("xf_groups")." g,".$xoopsDB -> prefix("xf_filemodule_monitor")." f,".$xoopsDB -> prefix("xf_frs_package")." p "."WHERE g.group_id=p.group_id AND g.status = 'A' "."AND p.package_id=f.filemodule_id "."AND f.user_id='".$xoopsUser -> getVar("uid")."' ORDER BY group_name DESC";
...
else
{
	...
	for ($i = 0; $i < $rows; $i ++)
	{
		if (unofficial_getDBResult($result, $i, 'group_id') != $last_group)
		{
			$content.= "<tr><td colspan='2'><b><a href='../xfmod/project/?".unofficial_getDBResult($result, $i, 'unix_group_name')."'>".$ts -> makeTboxData4Show(unofficial_getDBResult($result, $i, 'group_name'))."</a></td></tr>";
		}
...
$sql = "SELECT g.group_name,pgl.project_name,pgl.group_id,pt.group_project_id,pt.priority,pt.project_task_id,pt.summary,pt.percent_complete "."FROM ".$xoopsDB -> prefix("xf_groups")." g,".$xoopsDB -> prefix("xf_project_group_list")." pgl,".$xoopsDB -> prefix("xf_project_task")." pt,".$xoopsDB -> prefix("xf_project_assigned_to")." pat "."WHERE pt.project_task_id=pat.project_task_id "."AND pat.assigned_to_id='".$xoopsUser -> getVar("uid")."' "."AND pt.status_id='1' "."AND pgl.group_id=g.group_id "."AND pgl.group_project_id=pt.group_project_id "."AND g.status = 'A'"."ORDER BY group_name,project_name";
...
if (intval($xoopsForge['devsurvey']) != 100)
{
...
$sql = "SELECT bookmark_url,bookmark_title,bookmark_id FROM "
	.$xoopsDB->prefix("xf_user_bookmarks")
	." WHERE user_id='"
