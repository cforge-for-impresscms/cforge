<?php
	...
	* @version  $Id: HTML_Graphs.php,v 1.3 2004/01/26 18:56:51 devsupaul Exp $
	* @date  $Date: 2004/01/26 18:56:51 $
	*/
	 
	/*
	#######################################################################
	#
	#   $Source: /cvsroot/forge/xoops2/html/modules/xfmod/include/HTML_Graphs.php,v $
	# $Revision: 1.3 $
	#   $State: Exp $
	#
	...
	*/
	function start_graph($vals, $names)
	{
		...
		if ($vals["width"] != 0)
		{
			print ' width="' . $vals["width"] . '"';
		}
		if ($vals["background"])
		{
			print ' BACKGROUND="' . $vals["background"] . '"';
		}
		...
		if (($vals["vlabel"]) || ($vals["hlabel"]))
		{
			...
			print '<th><td align=CENTER valign="CENTER" ';
			 
			// If a background was choosen don't print cell BGCOLOR
			if (! $vals["background"])
			{
				print 'BGCOLOR="' . $vals["hbgcolor"] . '"';
			}
	...
	*/
	function hv_graph_defaults($vals)
	{
		...
		if (! $vals["vbgcolor"])
		{
			$vals["vbgcolor"] = "#FFFFFF";
		}
		if (! $vals["hbgcolor"])
		{
			$vals["hbgcolor"] = "#FFFFFF";
		}
		if (! $vals["cellpadding"])
		{
			$vals["cellpadding"] = 0;
		...
		if (! $vals["namebgcolor"])
		{
			$vals["namebgcolor"] = "#FFFFFF";
		}
		if (! $vals["valuebgcolor"])
		{
			$vals["valuebgcolor"] = "#FFFFFF";
		}
		if (! $vals["namefcolor"])
		{
			$vals["namefcolor"] = "#000000";
		}
		if (! $vals["valuefcolor"])
		{
			$vals["valuefcolor"] = "#000000";
		}
		if (! $vals["doublefcolor"])
		{
			$vals["doublefcolor"] = "#886666";
		}
...
function vertical_graph($names, $values, $bars, $vals)
{
	...
	for($i = 0; $i < SizeOf($values); $i++)
	{
		...
		if (! $vals["background"])
		{
			print ' BGCOLOR="' . $vals["valuebgcolor"] . '"';
		}
		print ">";
...
<td align="CENTER" valign="TOP" 
	...
	<?php
		...
		if (! $vals["background"])
		{
			print ' BGCOLOR="' . $vals["namebgcolor"] . '"';
		}
	?>
>
...
<th><td 
	<?php
		...
		if (ereg("^#", $dbars[$i]))
		{
			print 'BGCOLOR="' . $dbars[$i] . '">';
		}
		else
		{
			print 'BACKGROUND="' . $dbars[$i] . '">';
		}
	?>
<NOWRAP>
	<?php
...
function double_vertical_graph($names, $values, $bars, $vals, $dvalues, $dbars)
{
	...
	for($i = 0; $i < SizeOf($values); $i++)
	{
		...
		print '<table><th><td align="CENTER" valign="BOTTOM" ';
...
echo '<table align="LEFT" border=0 CELLPADDING=0 CELLSPACING=0><th>'."\r\n";
$prev_val=0;
$shown=0;
