<?php
	...
	*/
	...
	{
		$filename = ICMS_ROOT_PATH."/modules/xfmod/cache/xfcache_$name.xf";
		...
		while (!file_exists($filename) || (filesize($filename) <= 1) || ((time() - filemtime($filename)) > $time))
		{
			...
			else
			{
				...
				global $icmsForgeErrorHandler;
				$icmsForgeErrorHandler->setSystemError("Unable to open cache file for writing.");
			}
		}
	...
	function cache_display($name, $function, $time)
	{
		...
		while ((filesize($filename) <= 1) || ((time() - filemtime($filename)) > $time))
		{
			...
			else
			{
				// unable to open for reading
				global $icmsForgeErrorHandler;
				$icmsForgeErrorHandler->addError("Unable to open cache file for writing.");
				return cache_get_new_data($function);
			}
		}
		// file is now good, use it for return value
		if (!$rfh = fopen($filename, "rb"))
		{
			//bad filename
			return cache_get_new_data($function);
		}
		while (!flock($rfh, 1 + 4) && ($counter < 30))
		{
			// obtained non blocking shared lock
