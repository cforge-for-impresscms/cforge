<?php
	...
	*/
	...
	$icmsOption['template_main'] = 'project/admin/xfmod_editreleases.html';
	...
	*/
	...
	if (!$release_id)
	{
		...
		{
			if ($package_id)
			$content = "<strong>"._XF_PRJ_NORELEASESTHISPACKAGEDEFINED."</strong>";
			else
				$content = "<strong>"._XF_PRJ_NORELEASESDEFINED."</strong>";
		...
		else
		{
			...
			*/
			$content .= "
				...
				<td align='center'><span class='fg2'><strong>"._XF_PRJ_RELEASENAME."</strong></span></td>
			...
			for($i = 0; $i < $rows; $i++)
			{
				...
				$content .= "<tr class='".($i%2 > 0?"bg1":"bg3")."'>" ."<td>" ."<font size='-1'>" .$ts->makeTboxData4Show(unofficial_getDBResult($res, $i, 'release_name'))
				."[ <a href='editreleases.php?package_id=".$package_id."&release_id=".unofficial_getDBResult($res, $i, 'release_id')."&group_id=".$group_id."'>"._XF_PRJ_EDITTHISRELEASE."</a> ]" ."</font>" ."</td>" ."<td>" ."<font size='-1'>" .$ts->makeTboxData4Show(unofficial_getDBResult($res, $i, 'package_name'))
				."[ <a href='editpackages.php?group_id=".$group_id."'>"._XF_PRJ_EDITTHISPACKAGE."</a> ]" ."</font>" ."</td>" ."<td>" ."<font size='-1'>".unofficial_getDBResult($res, $i, 'status_name')."</font>" ."</td>" ."</tr>" ."</form>";
			}
	...
	*/
	if ($release_id)
	{
		...
		."</td>
			...
			<tr>
			<td colspan='2'>
			<strong>"._XF_PRJ_PASTENOTES.":</strong><br>
			...
			<th>
			<td colspan=2>
			<strong>"._XF_PRJ_PASTELOG.":</strong><br>
			...
			<th>
			<td colspan=2>
			<strong>"._XF_PRJ_PASTEDEPENDENCIES.":</strong><br>
		...
		else
		{
			$content .= "
				...
				<tr>
				<td colspan='2'>"._XF_PRJ_DISCLAIMER." <a href='#' onclick=\"window.open('".ICMS_URL."/modules/xfmod/include/contract.php','contract', 'width=750, height=350, resizable=yes, scrollbars=yess, toolbar=no, directories=no, location=no, status=no');\">"._XF_PRJ_DISCLAIMERLINK."</a></td>
				</tr>
				<tr>
				<td colspan=2>".sprintf(_XF_PRJ_MAX_UPLOAD, ini_get('upload_max_filesize'))."</td>
				</tr>
				<tr>
				<td><strong>"._XF_PRJ_FILENAME.":<strong></td>
				<td><input type='text' name='filename' maxlength=255 size='40'></td>
				</tr>
				<tr>
				<td><strong>"._XF_PRJ_FILESIZE.":<strong></td>
				<td><input type='text' name='file_size' maxlength='12' size='40'></td>
				</tr>
				<tr>
				<td><strong>"._XF_PRJ_LINK1.":<strong></td>
				<td><input type='text' name='file_url' maxlength='255' size='40'></td>
				</tr>
				<tr>
				<td></td><td><strong>- "._XF_PRJ_OR." -</strong></td>
				</tr>
				<tr>
				<td><strong>"._XF_PRJ_FILE1.":<strong></td>
		...
		if (!$frs->isError())
		{
			...
			if ($rows < 1)
			{
				$content .= "<strong>"._XF_PRJ_NOFILESINRELEASE."</strong>";
			}
			else
			{
				...
				for($x = 0; $x < $rows; $x++)
				{
					...
					{
						$sql = "SELECT count(file_id) FROM ".$icmsDB->prefix("xf_webservice_publish")." WHERE file_id=".unofficial_getDBResult($res, $x, 'file_id')." AND status='succeeded'";
						$myresult = $icmsDB->query($sql);
						list($mycount) = $icmsDB->fetchRow($myresult);
						if ($mycount == 0)
