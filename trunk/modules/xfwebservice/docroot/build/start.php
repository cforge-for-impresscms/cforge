...
$sql = 'SELECT * FROM '.$xoopsDB->prefix('xf_webservice_build')
	." WHERE user_id=$userID AND (status='pending' OR status='active')";
$result = $xoopsDB->queryF($sql);
if (!$result || $xoopsDB->getRowsNum($result) >= BUILD_QUOTA)
{
...
if (!$error)
{
	$dnsmapfile = XOOPS_ROOT_PATH.'/modules/xfmod/cvs/dnsmap.php';
...
$start = time();
$sql = 'INSERT INTO '.$xoopsDB->prefix('xf_webservice_build')
	.' (user_id,unix_group_name,target,cvs_host,cvs_modules,start_time,status,error) VALUES'
	." ($userID,'$name','$target','$cvshost','$cvsmodules',$start,'$status','$error')";
if (!$xoopsDB->queryF($sql))
{
	// Could not create build record.
	$tpl = TEMPLATE_CONTEXT.'error.tpl';
	$args['id'] = 214;
	$args['contents'] = _XFWEBSERVICE_ERROR_214.': '.$xoopsDB->error();
	echo fill_template($tpl, $args);
	return;
}
