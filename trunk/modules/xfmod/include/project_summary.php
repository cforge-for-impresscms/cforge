...
function project_get_survey_count($group_id)
{
	...
	$sql = "SELECT COUNT(*) AS count "
	      ."FROM ".$xoopsDB->prefix("xf_surveys")." "
				."WHERE is_active=1 "
...
function project_get_public_forum_count($group_id)
{
	...
	if($xoopsForge['forum_type']=="forum") {
		...
		$sql = "SELECT COUNT(*) AS count"
			." FROM ".$xoopsDB->prefix("xf_forum_nntp_list")
 ...
 * @param		int		The group ID
 */
include_once(XOOPS_ROOT_PATH."/modules/xfmod/newsportal/config.inc");
include_once(XOOPS_ROOT_PATH."/modules/xfmod/newsportal/newsportal.php");
function project_get_public_forum_message_count($group_id)
{
	...
	if($xoopsForge['forum_type']=="forum") {
		$sql = "SELECT COUNT(f.msg_id) AS count " .
				...
				" f,".$xoopsDB->prefix("xf_forum_group_list") .
				" fgl "
		...
		$sql = "SELECT forum_name from ".$xoopsDB->prefix("xf_forum_nntp_list")
				." WHERE group_id=$group_id";
...
function project_tabs($toptab, $group, $extra_text='')
{
	...
	if ( $project->isProject())
	{
		...
		// CVS
		//if ($project->usesCVS())
		if ($project->usesCVS() && ($project->anonCVS() || ($xoopsUser && $perm->isMember("user_id",$xoopsUser->getVar('uid'))))) $numtabs++;
	}
	...
	$content = "<BR/><table border='0' cellpadding='0' cellspacing='0' width='100%'><tr><td height='1' style='background-image:url(".XOOPS_THEME_URL."/".$xoopsConfig['theme_set']."/dotlinebg_horiz.gif)'><img src='".XOOPS_THEME_URL."/".$xoopsConfig['theme_set']."/spacer.gif' width='100%' height='1' border='0' alt=''></td><td width='20'></td></tr>";
	...
	{
		$content .= tab_entry( XOOPS_URL.'/modules/xfmod/community/members.php?group_id='.
			$group, '', _XF_G_MEMBERS, $toptab == 'members');
	}

	//newsbytes
	if ($project->usesNews())
	{
		$content .= tab_entry( XOOPS_URL.'/modules/xfmod/news/?group_id='.
			$group, '', _XF_G_NEWS, $toptab == 'news');
	}

	// Forums
	if ($project->usesForum())
	{
		$content .= tab_entry( XOOPS_URL.'/modules/xfmod/'.$xoopsForge['forum_type'].'/?group_id='.$group,
	...
	if ($project->usesFAQ())
	{
		$content .= tab_entry( XOOPS_URL.'/modules/xfmod/faqs/?group_id='.$group,
	...
	{
		if($project->usesTracker()){
			$content .= tab_entry( XOOPS_URL.'/modules/xfmod/tracker/?group_id='.$group,
			...
			for ($i = 0; $i < $rows; $i++)
			{
				...
				{
					//bug Tracker
					$content .= tab_entry( XOOPS_URL.'/modules/xfmod/tracker/?group_id=' .
						$group.'&atid='.unofficial_getDBResult($res,$i,'group_artifact_id'),
		...
		if ($project->usesPm())
		{
			$content .= tab_entry( XOOPS_URL.'/modules/xfmod/pm/?group_id='.$group,
	...
	if($project->usesSamples()) {
		$content .= tab_entry( XOOPS_URL.'/modules/xfmod/sample/?group_id='.
			$group, '', _XF_G_SAMPLE, $toptab=='sample');
	}

	// Doc Manager
	if ($project->usesDocman()) {
		$content .= tab_entry( XOOPS_URL.'/modules/xfmod/docman/?group_id='.
			$group, '', _XF_G_DOCS, $toptab == 'docman');
	}

	// Mailing Lists
	if ($project->usesMail()) {
		$content .= tab_entry( XOOPS_URL.'/modules/xfmod/maillist/?group_id='.
			$group,'',_XF_G_LISTS, $toptab == 'maillist');
	}

	// Surveys
	if ($project->usesSurvey()) {
		$content .= tab_entry( XOOPS_URL.'/modules/xfmod/survey/?group_id='
			.$group, '', _XF_G_SURVEYS, $toptab == 'surveys');
	}

	// Downloads
	if ( $project->isProject()) {
		$content .= tab_entry( $urlpath.'showfiles.php?group_id='. 
			$group, '', _XF_G_FILES, $toptab == 'downloads');
		...
		if ($project->usesCVS() && ($project->anonCVS() || ($xoopsUser && $perm->isMember("user_id", $xoopsUser->getVar('uid')))))
		{
			...
			*/
			...
			$content .= tab_entry( XOOPS_URL.'/modules/xfmod/cvs/cvspage.php/' .
				$project->getUnixName().'/', '', _XF_G_CVS,
				$toptab == 'CVS');
		}
	}
	...
	$content .= "</span></td></tr>";
	$content .= "<tr><td colspan='4' height='1'></td></tr><tr><td height='1' style='background-image:url(".XOOPS_THEME_URL."/".$xoopsConfig['theme_set']."/dotlinebg_horiz.gif)'><img src='".XOOPS_THEME_URL."/".$xoopsConfig['theme_set']."/spacer.gif' width='173' height='1' border='0' alt=''></td><td width='20'></td></tr></table>";
	$content .= "<p/>";
	return $content;
}
