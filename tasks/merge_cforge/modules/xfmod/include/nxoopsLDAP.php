<?php
	...
	include_once(ICMS_ROOT_PATH."/kernel/object.php");
	include_once(ICMS_ROOT_PATH."/kernel/group.php");
	include_once(ICMS_ROOT_PATH."/class/icmsUser.php");
	...
	class nxoopsLDAP {
		...
		function connect()
		{
			global $icmsConfig;
			$this->error = null;
			$server = ($icmsConfig['ldapserverport'] == 389) ? "ldap" :
			 "ldaps";
			$server .= "://" . $icmsConfig['ldapserver'];
			$this->conn = ldap_connect($server, $icmsConfig['ldapserverport']);
			if (!$this->conn)
			{
				$this->conn = null;
		...
		function doBind($userDN, $password)
		{
			...
			{
				$userDN .= " LDAP Server: " . $icmsConfig['ldapserver'];
		...
		{
			global $icmsConfig;
			return $this->doBind($icmsConfig['ldapadmin'], $icmsConfig['ldapadminpass']);
		}
		...
		function getUser($userName, $includeGroups = false)
		{
			...
			$this->error = null;
			$userObj = null;
			$entryDN = $icmsConfig['ldapusercont'];
			$filter = "(cn=".$userName.")";
		...
		function addPosixToUser($userName, $userDN, $uidNumber)
		{
			...
			if (!$res)
			{
				return $this->returnLDAPFailure("Adding posixAccount to user: " . $userDN);
			}
		...
		function modUserGroupRelation($userObj, $grpName, $doDel = false)
		{
			...
			$userDN = $ldapUser->dn;
			$projBase = "ou=projects,ou=nforge,".$icmsConfig['ldaproot'];
		...
		function getAvailCVSServer($allServers = false)
		{
			...
			$base = "ou=cvsservers,ou=nforge,".$icmsConfig['ldaproot'];
		...
		function getProjectCVSServer($projectName)
		{
			...
			$entryDN = "cn=$projectName,ou=projects,ou=nforge,".$icmsConfig['ldaproot'];
		...
		function addCVSServerAction($cvsServerDN, $identifier, $tranType, $tranbuf = null)
		{
			...
			$count = unofficial_getDBResult($icmsDB->query($sql), 0, 'count');
		...
		function createProject($projectName, $gidNumber, $anonAllowed)
		{
			...
			if ($projectExists)
			{
				...
				$entry['nforgecvsserverref'] = $cvsserver->dn;
				$entry['nforgeanonymousallowed'] = ($anonAllowed) ? "TRUE" :
				 "FALSE";
		...
		function setAnonAllowed($projectName, $anonFlag = true)
		{
			...
			$entryDN = "cn=$projectName,ou=projects,ou=nforge,".$icmsConfig['ldaproot'];
			$entry = array();
			$entry['nforgeanonymousallowed'] = ($anonFlag) ? "TRUE" :
			 "FALSE";
			 
			// Modify the anon flag on the project
			if (!ldap_mod_replace($this->conn, $entryDN, $entry))
			{
				return $this->returnLDAPFailure("Change anonymous flag for " . " project: $entryDN");
			}
