<?php
	...
	$devname = $icmsUser ? $icmsUser->getVar("uname")
	:
	 "your-user-name";
	...
	if ($project->anonCVS())
	{
		...
		echo '<ul><li><a href="'.ICMS_URL.'/modules/xfmod/cvs/cvsbrowse.php/'.$projname.'/" style="text-decoration: none; font-weight: 700">';
	...
	if ($icmsUser)
	{
		echo "  You can upload you public keys from <a href='".ICMS_URL."/modules/xfaccount/pubkeys.php'>your account page</a>.</p>";
	}
	else
	{
	...
	echo "<p align='left'>If you are a registered developer of the $projname". " project, you may use all of the functionality provided by CVS.<br/>". " For the full documentation on CVS, you can visit their WEB site at". ' <a href="http://www.cvshome.org/">http://www.cvshome.org.</a><br/>'. "<p align='left'>Keep in mind that whenever you issue any CVS". " command on your project's CVS repository, you will be required". " to enter your site password.</p>". "<p align='left'>The most common things you will be doing with". " CVS are importing your project, checking it out, updating". " your local copy with the repository and checking in.". "<p align='left'>If your project's files are not yet in CVS,". " you must start with the CVS import. To do this,". " you should change to the topmost directory of your project's". " files and enter something like the following:". "</p><P align='left'><strong>";
	 
	echo "cvs -z3 -d:ext:$devname@$cvsdns:/cvsroot/$projname import". ' -m <em>"Init msg" rootdirname vendor starttag</em></strong>';
	 
	echo '<p align="left">In the above command, we are telling CVS to '. " create the directory <em>./rootdirname</em> in the project's". " CVS repository and copy everything from the current directory and". " any sub-directories into it.". ' The <em>"Init msg", vendor</em> and <em>starttag</em> can be any'. ' text you like. The <em>"Init msg"</em> is used to specify a message'. " that will be associated with the import. <em>vendor</em> is used to". " specify the company that owns the project. <em>starttag</em> is the". " CVS tag name that will be associated with the import</p>";
	...
	echo "<p align='left'>Now you can get your own private copy of all ". "the project files in CVS by doing a check-out. You can do this". " with the following command:</p><p align='left'><strong>". "cvs -z3 -d:ext:$devname@$cvsdns:/cvsroot/$projname co <em>modulename</em></strong></p>". "<p align='left'><em>modulename</em> would be replace with the value you". " used for the <em>./rootdirname</em> parameter when doing the import.</p>";
	 
	echo "<p align='left'>The CVS update command is used for downloading all changes". " that have been made to files in the CVS repository to your local copy.". " If another developer has changed a file that you have also changed in". " your local copy, the changes will be merged together. Sometimes a merge". " is required but cannot be completed. This condition produces a conflict.". " If there are any conflicts during the update process, they will appear in". " the output of the command. Under almost all conditions, an update operation". " must be performed before a check-in operation. If you check-in before doing". " an update, you run the risk of overwriting someone else's changes!". " To perform a CVS update, you should change to the root directory of your". " local copy of the project files and issue the following command:". "</p><p align='left'><strong>". "cvs -z3 -d:ext:$devname@$cvsdns:/cvsroot/$projname update</strong></p>";
	 
	 
	echo "<p align='left'>At some point in time, you will want to check-in the changes". " that you have made to your local copy of the project files. This is". " a multi-phased process that should be as follows:". "<ul><li>Do a CVS update operation(described above).</li>". "<li>Fix any problems that may have surfaced from the update,". " such as failed merges(conflicts), etc...</li>". "<li>Do the actual CVS check-in operation</li></ul>". "<p align='left'>When you are ready to do the CVS check-in, change to any directory". " in your local copy of the project files and issue the following command:". "</p><p align='left'><strong>". "cvs -z3 -d:ext:$devname@$cvsdns:/cvsroot/$projname commit</strong></p>". "<p align='left'>This will check-in all changes in the current directory". " and any of it's sub-directories.</p>";
	 
	echo "<p align='left'>Again, CVS has much more functionality than what is". " described here. We encorage you to check out the documentation on". ' their site: <a href="http://www.cvshome.org" target="developer">CVS Home</a>.</p>';
