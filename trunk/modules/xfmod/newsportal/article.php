...
<?php
  ...
  if ((!$readonly) && ($message))
  	if($xoopsUser!=null)
		echo '[<a href="'.$file_post.'?group_id='.$group_id.'&type=reply&msg_id='.urlencode($msg_id).
			 '&group='.urlencode($mygroup).'">'.$text_article["button_answer"].'</a>]'."\n";
	else
  ...
  if($perm->isForumAdmin())
	echo '[<a href="'.$file_cancel.'?group_id='.$group_id.'&type=reply&msg_id='.urlencode($msg_id).
	   '&group='.urlencode($mygroup).'">Delete Message</a>]'."\n";
