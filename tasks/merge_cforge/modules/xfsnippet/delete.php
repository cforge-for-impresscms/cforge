<?php
	...
	*/
	...
	if ($icmsUser)
	{
		...
		if ($type == 'frompackage' && $snippet_version_id && $snippet_package_version_id)
		{
			...
			*/
			...
			else
			{
				...
				$result = $icmsDB->queryF("DELETE FROM ".$icmsDB->prefix("xf_snippet_package_item")." " ."WHERE snippet_version_id='$snippet_version_id' " ."AND snippet_package_version_id='$snippet_package_version_id'");
		...
		else if ($type == 'snippet' && $snippet_version_id)
		{
			...
			*/
			...
			$result = $icmsDB->queryF("SELECT * FROM ".$icmsDB->prefix("xf_snippet_version")." " ."WHERE snippet_version_id='$snippet_version_id' " ."AND submitted_by='".$icmsUser->getVar("uid")."'");
			...
			else
			{
				...
				//do the delete
				$result = $icmsDB->queryF("DELETE FROM ".$icmsDB->prefix("xf_snippet_version")." " ."WHERE snippet_version_id='$snippet_version_id' " ."AND submitted_by='".$icmsUser->getVar("uid")."'");
				 
				//see if any versions of this snippet are left
				$result = $icmsDB->query("SELECT * FROM ".$icmsDB->prefix("xf_snippet_version")." WHERE snippet_id='$snippet_id'");
				if (!$result || $icmsDB->getRowsNum($result) < 1)
				{
					...
					$result = $icmsDB->queryF("DELETE FROM ".$icmsDB->prefix("xf_snippet")." WHERE snippet_id='$snippet_id'");
				}
		...
		else if ($type == 'package' && $snippet_package_version_id)
		{
			...
			*/
			...
			else
			{
				...
				$result = $icmsDB->queryF("DELETE FROM ".$icmsDB->prefix("xf_snippet_package_version")." " ."WHERE submitted_by='".$icmsUser->uid()."' " ."AND snippet_package_version_id='$snippet_package_version_id'");
				 
				//delete snippet_package_items
				$result = $icmsDB->queryF("DELETE FROM ".$icmsDB->prefix("xf_snippet_package_item")." " ."WHERE snippet_package_version_id='$snippet_package_version_id'");
				 
				//see if any versions of this package remain
				$result = $icmsDB->query("SELECT * FROM ".$icmsDB->prefix("xf_snippet_package_version")." " ."WHERE snippet_package_id='$snippet_package_id'");
				 
				if (!$result || $icmsDB->getRowsNum($result) < 1)
				{
					//since no versions of this package remain,
					//delete the main package even if the user didn't create it
					$result = $icmsDB->queryF("DELETE FROM ".$icmsDB->prefix("xf_snippet_package")." WHERE snippet_package_id='$snippet_package_id'");
					redirect_header(ICMS_URL."/modules/xfsnippet/");
				}
	...
	else
	{
		...
		$xoopsForgeErrorHandler->setSystemError(_NOPERM . "called from ".__FILE__." line ".__LINE__ );
	}
